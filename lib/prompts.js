...
When diagrams, charts, tables, or visual elements are present or implied, provide a self-contained visual (inline SVG, an HTML table, or styled HTML — no external assets, no scripts). The visual is rendered inside a light-themed card, so you may assume a near-white background; if you specify any background colors on cells or shapes, also set an explicit dark text color so the content stays readable. After the visual, describe what it shows and explain what it represents, why it matters, and how it connects to surrounding concepts.
